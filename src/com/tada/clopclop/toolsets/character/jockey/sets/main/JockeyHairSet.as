package com.tada.clopclop.toolsets.character.jockey.sets.main
{
	...
	public class JockeyHairSet extends EventDispatcher implements JockeyCharacterSets
	{
		...
		public function JockeyHairSet(instanceID:int, meshContainer3D:ObjectContainer3D)
		{ 
		...
		private function loadAsset():void{
			...
			if (_jockeyGR.xmlLoaded[JockeyGR.HAIR_SET] == true){
				var params:Object = new Object();
				params.partType = JockeyGR.HAIR_SET;
		...
		private function onXMLLoaded (evt:JockeyEvent):void{
			if (evt.params.partType == JockeyGR.HAIR_SET){
				_jockeyGR.removeOnXMLLoaded(onXMLLoaded);
				_readyToLoadMesh = false;
				_jockeyGR.loadMesh(_instanceID, JockeyGR.HAIR_SET, _meshIndex, _meshInstanceCode);
		...
		private function onMeshLoaded (evt:JockeyEvent):void{
			if (evt.params.partType == JockeyGR.HAIR_SET && evt.params.assetID == _instanceID){
				if (_meshContainer.getChildByName("jockeyHair") != null){
					_meshContainer.removeChildByName("jockeyHair");
				}
				_assetLoaded = true;
				_jockeyGR.removeOnMeshLoaded(onMeshLoaded);
				_mesh = _jockeyGR.getMesh(_instanceID, JockeyGR.HAIR_SET, _meshIndex);
		...
		public function setInitialIndexes():void{
			_meshIndex = JockeyGR.MESHHAIR_DEFAULT;
			_textureIndex = JockeyGR.MESHHAIR_DEFAULT;
		}
		...
		private function textureLayeringManager(layer:String = ""):void{
			...
			if (layer == JockeyGR.TEXLAYER_MAIN) {
				...
				_textureMaterial = _jockeyGR.getTexture(JockeyGR.HAIR_SET, _textureIndex);
