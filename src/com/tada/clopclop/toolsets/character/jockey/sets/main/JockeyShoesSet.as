package com.tada.clopclop.toolsets.character.jockey.sets.main
{
	...
	public class JockeyShoesSet extends EventDispatcher implements JockeyCharacterSets
	{
		...
		public function JockeyShoesSet(instanceID:int, meshContainer3D:ObjectContainer3D)
		{ 
		...
		private function loadAsset():void{
			...
			if (_jockeyGR.xmlLoaded[JockeyGR.SHOES_SET] == true){
				var params:Object = new Object();
				params.partType = JockeyGR.SHOES_SET;
		...
		private function onXMLLoaded (evt:JockeyEvent):void{
			if (evt.params.partType == JockeyGR.SHOES_SET){
				_jockeyGR.removeOnXMLLoaded(onXMLLoaded);
				_readyToLoadMesh = false;
				_jockeyGR.loadMesh(_instanceID, JockeyGR.SHOES_SET, _meshIndex, _meshInstanceCode);
		...
		private function onMeshLoaded (evt:JockeyEvent):void{
			if (evt.params.partType == JockeyGR.SHOES_SET && evt.params.assetID == _instanceID){
				if (_meshContainer.getChildByName("jockeyShoes") != null){
					_meshContainer.removeChildByName("jockeyShoes");
				}
				_assetLoaded = true;
				_jockeyGR.removeOnMeshLoaded(onMeshLoaded);
				_mesh = _jockeyGR.getMesh(_instanceID, JockeyGR.SHOES_SET, _meshIndex);
		...
		public function setInitialIndexes():void{
			_meshIndex = JockeyGR.MESHSHOES_DEFAULT;
			_textureIndex = JockeyGR.MESHSHOES_DEFAULT;
		}
		...
		private function textureLayeringManager(layer:String = ""):void{
			...
			if (layer == JockeyGR.TEXLAYER_MAIN) {
				...
				_textureMaterial = _jockeyGR.getTexture(JockeyGR.SHOES_SET, _textureIndex);
