package com.tada.clopclop.toolsets.character.jockey.sets.main
{
	...
	public class JockeyTopSet extends EventDispatcher implements JockeyCharacterSets
	{
		...
		public function JockeyTopSet(instanceID:int, meshContainer3D:ObjectContainer3D)
		{ 
		...
		private function loadAsset():void{
			...
			if (_jockeyGR.xmlLoaded[JockeyGR.TOP_SET] == true){
				var params:Object = new Object();
				params.partType = JockeyGR.TOP_SET;
		...
		private function onXMLLoaded (evt:JockeyEvent):void{
			if (evt.params.partType == JockeyGR.TOP_SET){
				_jockeyGR.removeOnXMLLoaded(onXMLLoaded);
				_readyToLoadMesh = false;
				_jockeyGR.loadMesh(_instanceID, JockeyGR.TOP_SET, _meshIndex, _meshInstanceCode);
		...
		private function onMeshLoaded (evt:JockeyEvent):void{
			if (evt.params.partType == JockeyGR.TOP_SET && evt.params.assetID == _instanceID){
				if (_meshContainer.getChildByName("jockeyTop") != null){
					_meshContainer.removeChildByName("jockeyTop");
				}
				_assetLoaded = true;
				_jockeyGR.removeOnMeshLoaded(onMeshLoaded);
				_mesh = _jockeyGR.getMesh(_instanceID, JockeyGR.TOP_SET, _meshIndex);
		...
		public function setInitialIndexes():void{
			_meshIndex = JockeyGR.MESHTOP_DEFAULT;
			_textureIndex = JockeyGR.MESHTOP_DEFAULT;
		}
		...
		private function textureLayeringManager(layer:String = ""):void{
			...
			if (layer == JockeyGR.TEXLAYER_MAIN) {
				...
				_textureMaterial = _jockeyGR.getTexture(JockeyGR.TOP_SET, _textureIndex);
