package com.tada.clopclop.toolsets.character.jockey.sets.equipment
{
	...
	public class JockeyHeadSet extends EventDispatcher implements JockeyCharacterSets
	{
		...
		public function JockeyHeadSet(instanceID:int, meshContainer3D:ObjectContainer3D)
		{ 
		...
		private function loadAsset():void{
			...
			if (_jockeyGR.xmlLoaded[JockeyGR.HEAD_SET] == true){
				var params:Object = new Object();
				params.partType = JockeyGR.HEAD_SET;
		...
		private function onXMLLoaded (evt:JockeyEvent):void{
			if (evt.params.partType == JockeyGR.HEAD_SET){
				_jockeyGR.removeOnXMLLoaded(onXMLLoaded);
				_readyToLoadMesh = false;
				_jockeyGR.loadMesh(_instanceID, JockeyGR.HEAD_SET, _meshIndex, _meshInstanceCode);
		...
		private function onMeshLoaded (evt:JockeyEvent):void{
			if (evt.params.partType == JockeyGR.HEAD_SET && evt.params.assetID == _instanceID){
				if (_meshContainer.getChildByName("jockeyHead") != null){
					_meshContainer.removeChildByName("jockeyHead");
				}
				_assetLoaded = true;
				_jockeyGR.removeOnMeshLoaded(onMeshLoaded);
				_mesh = _jockeyGR.getMesh(_instanceID, JockeyGR.HEAD_SET, _meshIndex);
		...
		public function setInitialIndexes():void{
			_meshIndex = JockeyGR.MESHHEAD_001;
			_textureIndex = JockeyGR.TEXHEAD_001_0001;
		}
		...
		private function textureLayeringManager(layer:String = ""):void{
			...
			if (layer == JockeyGR.TEXLAYER_MAIN) {
				...
				_textureMaterial = _jockeyGR.getTexture(JockeyGR.HEAD_SET, _textureIndex);
