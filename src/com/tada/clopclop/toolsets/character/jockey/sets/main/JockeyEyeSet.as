...
{	
	...
	public class JockeyEyeSet extends EventDispatcher implements JockeyCharacterSets
	{
		...
		public function JockeyEyeSet(instanceID:int, meshContainer3D:ObjectContainer3D)
		{ 
		...
		private function loadAsset():void{
			...
			if (_jockeyGR.xmlLoaded[JockeyGR.EYE_SET] == true){
				var params:Object = new Object();
				params.partType = JockeyGR.EYE_SET;
		...
		private function onXMLLoaded (evt:JockeyEvent):void{
			if (evt.params.partType == JockeyGR.EYE_SET){
				_jockeyGR.removeOnXMLLoaded(onXMLLoaded);
				_readyToLoadMesh = false;
				_jockeyGR.loadMesh(_instanceID, JockeyGR.EYE_SET, _meshIndex, _meshInstanceCode);
		...
		private function onMeshLoaded (evt:JockeyEvent):void{
			if (evt.params.partType == JockeyGR.EYE_SET && evt.params.assetID == _instanceID){
				if (_meshContainer.getChildByName("jockeyEye") != null){
					_meshContainer.removeChildByName("jockeyEye");
				}
				_assetLoaded = true;
				_jockeyGR.removeOnMeshLoaded(onMeshLoaded);
				_mesh = _jockeyGR.getMesh(_instanceID, JockeyGR.EYE_SET, _meshIndex);
		...
		public function setInitialIndexes():void{
			_meshIndex = JockeyGR.MESHEYE_DEFAULT;
			_textureIndex = JockeyGR.MESHEYE_DEFAULT;
		}
		...
		private function textureLayeringManager(layer:String = ""):void{
			...
			if (layer == JockeyGR.TEXLAYER_MAIN) {
				...
				_textureMaterial = _jockeyGR.getTexture(JockeyGR.EYE_SET, _textureIndex);
