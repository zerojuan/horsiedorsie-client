package com.tada.clopclop.toolsets.character.jockey.sets.equipment
{
	...
	public class JockeyAccessorySet extends EventDispatcher implements JockeyCharacterSets
	{
		...
		public function JockeyAccessorySet(instanceID:int, meshContainer3D:ObjectContainer3D)
		{ 
		...
		private function loadAsset():void{
			...
			if (_jockeyGR.xmlLoaded[JockeyGR.ACC_SET] == true){
				var params:Object = new Object();
				params.partType = JockeyGR.ACC_SET;
		...
		private function onXMLLoaded (evt:JockeyEvent):void{
			if (evt.params.partType == JockeyGR.ACC_SET){
				_jockeyGR.removeOnXMLLoaded(onXMLLoaded);
				_readyToLoadMesh = false;
				_jockeyGR.loadMesh(_instanceID, JockeyGR.ACC_SET, _meshIndex, _meshInstanceCode);
		...
		private function onMeshLoaded (evt:JockeyEvent):void{
			if (evt.params.partType == JockeyGR.ACC_SET && evt.params.assetID == _instanceID){
				if (_meshContainer.getChildByName("jockeyAcc") != null){
					_meshContainer.removeChildByName("jockeyAcc");
				}
				_assetLoaded = true;
				_jockeyGR.removeOnMeshLoaded(onMeshLoaded);
				_mesh = _jockeyGR.getMesh(_instanceID, JockeyGR.ACC_SET, _meshIndex);
		...
		public function setInitialIndexes():void{
			_meshIndex = JockeyGR.MESHACC_001;
			_textureIndex = JockeyGR.MESHACC_001;
		}
		...
		private function textureLayeringManager(layer:String = ""):void{
			...
			if (layer == JockeyGR.TEXLAYER_MAIN) {
				...
				_textureMaterial = _jockeyGR.getTexture(JockeyGR.ACC_SET, _textureIndex);
