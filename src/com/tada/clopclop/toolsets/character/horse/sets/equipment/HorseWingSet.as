package com.tada.clopclop.toolsets.character.horse.sets.equipment
{
	...
	public class HorseWingSet extends EventDispatcher implements HorseCharacterSets
	{
		...
		public function HorseWingSet(instanceID:int, meshContainer:ObjectContainer3D)
		{ 
		...
		private function loadAsset():void{
			...
			if (_horseGR.xmlLoaded[HorseGR.WING_SET] == true){
				var params:Object = new Object();
				params.partType = HorseGR.WING_SET;
		...
		private function onXMLLoaded (evt:HorseEvent):void{
			if (evt.params.partType == HorseGR.WING_SET){
				_horseGR.removeOnXMLLoaded(onXMLLoaded);
				_readyToLoadMesh = false;
				_horseGR.loadMesh(_instanceID, HorseGR.WING_SET, _meshIndex, _meshInstanceCode);
		...
		private function onMeshLoaded (evt:HorseEvent):void{
			if (evt.params.partType == HorseGR.WING_SET && evt.params.assetID == _instanceID){
				_assetLoaded = true;
				_horseGR.removeOnMeshLoaded(onMeshLoaded);
				_mesh = _horseGR.getMesh(_instanceID, HorseGR.WING_SET, _meshIndex);
		...
		public function setInitialIndexes():void{
			_meshIndex = HorseGR.MESHWING_001;
			_textureIndex = HorseGR.TEXWING_001_0001;
		}
		...
		private function textureLayeringManager(layer:String = ""):void{
			...
			if (layer == HorseGR.TEXLAYER_MAIN) {
				...
				_textureMaterial = _horseGR.getTexture(HorseGR.WING_SET, _textureIndex);
