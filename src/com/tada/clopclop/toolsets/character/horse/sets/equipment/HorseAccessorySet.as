package com.tada.clopclop.toolsets.character.horse.sets.equipment
{
	...
	public class HorseAccessorySet extends EventDispatcher implements HorseCharacterSets
	{	private var _rotationHolder:Number = 0;
		...
		public function HorseAccessorySet(instanceID:int, meshContainer:ObjectContainer3D)
		{ 
		...
		private function loadAsset():void{
			...
			if (_horseGR.xmlLoaded[HorseGR.ACC_SET] == true){
				var params:Object = new Object();
				params.partType = HorseGR.ACC_SET;
		...
		private function onXMLLoaded (evt:HorseEvent):void{
			if (evt.params.partType == HorseGR.ACC_SET && evt.params.assetID == _instanceID){
				_horseGR.removeOnXMLLoaded(onXMLLoaded);
				_readyToLoadMesh = false;
				_horseGR.loadMesh(_instanceID, HorseGR.ACC_SET, _meshIndex, _meshInstanceCode);
		...
		private function onMeshLoaded (evt:HorseEvent):void{
			if (evt.params.partType == HorseGR.ACC_SET){
				if (_meshContainer.getChildByName("horseAcc") != null){
					_meshContainer.removeChildByName("horseAcc");
				}
				_assetLoaded = true;
				_horseGR.removeOnMeshLoaded(onMeshLoaded);
				_mesh = _horseGR.getMesh(_instanceID, HorseGR.ACC_SET, _meshIndex);
		...
		private function textureLayeringManager(layer:String = ""):void{
			...
			if (layer == HorseGR.TEXLAYER_MAIN) {
				...
				_textureMaterial = _horseGR.getTexture(HorseGR.EYE_SET, _textureIndex);
