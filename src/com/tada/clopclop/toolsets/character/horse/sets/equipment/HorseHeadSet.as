package com.tada.clopclop.toolsets.character.horse.sets.equipment
{
	...
	public class HorseHeadSet extends EventDispatcher implements HorseCharacterSets
	{
		...
		public function HorseHeadSet(instanceID:int, meshContainer:ObjectContainer3D)
		{ 
		...
		private function loadAsset():void{
			...
			if (_horseGR.xmlLoaded[HorseGR.HEAD_SET] == true){
				var params:Object = new Object();
				params.partType = HorseGR.HEAD_SET;
		...
		private function onXMLLoaded (evt:HorseEvent):void{
			if (evt.params.partType == HorseGR.HEAD_SET){
				_horseGR.removeOnXMLLoaded(onXMLLoaded);
				_readyToLoadMesh = false;
				_horseGR.loadMesh(_instanceID, HorseGR.HEAD_SET, _meshIndex, _meshInstanceCode);
		...
		private function onMeshLoaded (evt:HorseEvent):void{
			if (evt.params.partType == HorseGR.HEAD_SET && evt.params.assetID == _instanceID){
				if (_meshContainer.getChildByName("horseHead") != null){
					_meshContainer.removeChildByName("horseHead");
				}
				_assetLoaded = true;
				_horseGR.removeOnMeshLoaded(onMeshLoaded);
				_mesh = _horseGR.getMesh(_instanceID, HorseGR.HEAD_SET, _meshIndex);
		...
		public function setInitialIndexes():void{
			_meshIndex = HorseGR.MESHHEAD_001;
			_textureIndex = HorseGR.TEXHEAD_001_0001;
		}
		...
		private function textureLayeringManager(layer:String = ""):void{
			...
			if (layer == HorseGR.TEXLAYER_MAIN) {
				...
				_textureMaterial = _horseGR.getTexture(HorseGR.HEAD_SET, _textureIndex);
		 ...
		 * in viehead when the character set is integrated with other mesh. 
		 */
