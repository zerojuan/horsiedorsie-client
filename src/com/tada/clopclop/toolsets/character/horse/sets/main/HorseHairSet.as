package com.tada.clopclop.toolsets.character.horse.sets.main
{
	...
	public class HorseHairSet extends EventDispatcher implements HorseCharacterSets
	{	
		...
		private var _codeInstance:String = "";
		...
		public function HorseHairSet(instanceID:int, meshContainer:ObjectContainer3D)
		{ 
		...
		private function loadAsset():void{
			...
			if (_horseGR.xmlLoaded[HorseGR.HAIR_SET] == true){
				var params:Object = new Object();
				params.partType = HorseGR.HAIR_SET;
		...
		private function onXMLLoaded (evt:HorseEvent):void{
			if (evt.params.partType == HorseGR.HAIR_SET){
				_readyToLoadMesh = false;
				_horseGR.removeOnXMLLoaded(onXMLLoaded);
				_horseGR.loadMesh(_instanceID, HorseGR.HAIR_SET, _meshIndex, _meshInstanceCode);
		...
		private function onMeshLoaded (evt:HorseEvent):void{
			if (evt.params.partType == HorseGR.HAIR_SET && evt.params.assetID == _instanceID){
				if (_meshContainer.getChildByName("horseHair") != null){
					_meshContainer.removeChildByName("horseHair");
				}
				_assetLoaded = true;
				_horseGR.removeOnMeshLoaded(onMeshLoaded);
				_mesh = _horseGR.getMesh(_instanceID, HorseGR.HAIR_SET, _meshIndex);
		...
		public function setInitialIndexes():void{
			_meshIndex = HorseGR.MESHHAIR_DEFAULT;
			_textureIndex = HorseGR.TEXHAIR_DEFAULT;
		}
		...
		private function textureLayeringManager(layer:String = ""):void{
			...
			if (layer == HorseGR.TEXLAYER_MAIN) {
				...
				_textureMaterial = _horseGR.getTexture(HorseGR.HAIR_SET, _textureIndex);
