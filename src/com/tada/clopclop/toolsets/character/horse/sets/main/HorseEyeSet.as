package com.tada.clopclop.toolsets.character.horse.sets.main
{
	...
	public class HorseEyeSet extends EventDispatcher implements HorseCharacterSets
	{
		...
		public function HorseEyeSet(instanceID:int, meshContainer:ObjectContainer3D)
		{ 
		...
		private function loadAsset():void{
			...
			if (_horseGR.xmlLoaded[HorseGR.EYE_SET] == true){
				var params:Object = new Object();
				params.partType = HorseGR.EYE_SET;
		...
		private function onXMLLoaded (evt:HorseEvent):void{
			if (evt.params.partType == HorseGR.EYE_SET){
				_horseGR.removeOnXMLLoaded(onXMLLoaded);
				_readyToLoadMesh = false;
				_horseGR.loadMesh(_instanceID, HorseGR.EYE_SET, _meshIndex, _meshInstanceCode);
		...
		private function onMeshLoaded (evt:HorseEvent):void{
			if (evt.params.partType == HorseGR.EYE_SET && evt.params.assetID == _instanceID){
				if (_meshContainer.getChildByName("horseEye") != null){
					_meshContainer.removeChildByName("horseEye");
				}
				_assetLoaded = true;
				_horseGR.removeOnMeshLoaded(onMeshLoaded);
				_mesh = _horseGR.getMesh(_instanceID, HorseGR.EYE_SET, _meshIndex);
		...
		public function setInitialIndexes():void{
			_meshIndex = HorseGR.MESHEYE_DEFAULT;
			_textureIndex = HorseGR.TEXEYE_DEFAULT;
		}
